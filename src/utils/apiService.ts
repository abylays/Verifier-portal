...
export default class ApiService {
  ...
  static async verifySharedResponseToken(credentialShareRequestToken: string, credentialShareResponseToken:string) {
        const credentialParams = { credentialShareRequestToken, credentialShareResponseToken }
        const {data} = await verifierApi.post(endpoints.VERIFIER_VERIFY_PRESENTATION, credentialParams);
        return data;
    }

    static async createSharedRequestToken(credentialShareRequestToken: string) {
        const {data} = await verifierApi.post(endpoints.BUILD_CREDENTIAL_REQUEST, credentialShareRequestToken);
        return data;
    }
    static async generateSharedRequestToken(credentialShareRequestToken: string) {
        const {data} = await cloudWalletApi.post(endpoints.GENERATE_CREDENTIAL_REQUEST_TOKEN, credentialShareRequestToken);
        return data;
    }
  /**
